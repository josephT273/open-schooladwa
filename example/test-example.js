...
async function testOpenLibrary() {
    ...
    try {
        ...
        console.log("✅ Open Library API working:", result.numFound > 0 ? `${result.numFound} books found` : "No books found");
    } catch (error) {
...
async function testGoogleBooks() {
    ...
    try {
        ...
        console.log("✅ Google Books API working:", result.totalItems > 0 ? `${result.totalItems} books found` : "No books found");
    } catch (error) {
...
async function testGoogleAI() {
    ...
    if (!process.env.GOOGLE_API) {
        console.log("⚠️  Google API key not found. Set GOOGLE_API environment variable to test.");
        return;
    }
